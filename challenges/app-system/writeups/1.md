...
sys.stdout.write('\ncat .passwd\n') # sends a command to the shell and why is there even \n ? and what takes it 44 bytes is consumed by fgets() and after that fgets puts \0 null terminator so its limit hits and then bash pops so \ncat .passwd\n stdin is woned by bash and first \n means nothin junk bash reads line based input so empty command bash ignores it but its like preparing bash guarantees bash is fully ready before the real command hy the second \n ? cuz Bash executes commands only after Enter cat .passwd without \n would just sit in the input buffer.. 
