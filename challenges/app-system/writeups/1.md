...
sys.stdout.write('\ncat .passwd\n') # sends a command to the shell that will open if check == 0xdeadbeef.
...
**How it worked is Python script acts as the stdin for ch13. When ch13 calls system("/bin/bash"), the spawned /bin/bash inherits the same stdin/stdout, so any remaining input written by Python is consumed by bash as commands.**
